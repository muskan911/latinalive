...
with tab1:
    ...
    for i, p in enumerate(portfolio["panels"]):
        ...
        if up:
            img_dir = "Week6/storage/portfolios"
        ...
        if st.button(f"Generate Audio Panel {i+1}", key=f"audio_{i}"):
            audio_dir = "Week6/audio"
            os.makedirs(audio_dir, exist_ok=True)
            audio_path = os.path.join(audio_dir, f"{student_id}_panel{i+1}.mp3")
            from utils.audio import generate_audio
            generate_audio(p["latin"], audio_path)
...
with tab5:
    ...
    for i, p in enumerate(portfolio.get("panels", [])):
        if p.get("audio_path") and os.path.exists(p["audio_path"]):
            st.markdown(f"Panel {i+1} — Audio available")
